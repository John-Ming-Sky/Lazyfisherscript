...
(async function () {
    ...
    const CatchTiming = new class {
         ...
         * - 败北（红）：5~6% 不达标 / 98~99% 稀有(差一点上罕见) / 99.9~99.94 罕见(差一点上传说)
         */
        specialCatchMark(score) {
            ...
            if (score >= 99.9 && score <= 99.94) return { text: '败北', color: '#ef4444' };
            return null;
        }
...
function initCatchSummary() {
    (function() {
        ...
        function parseCard(card) {
            ...
            return { type: isCatch ? '上鱼' : '脱钩', fishName, grade, weight, interval, timestamp };
        }
