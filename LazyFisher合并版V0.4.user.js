...
// @author       大白鲨 / 天雨灵泽 / LazyFisher / 土豆土豆斑 / Cursor merged
// @license      MIT
...
(async function () {
        ...
        recordPanel() {
            ...
            function refresh() {
                ...
                tableBody.innerHTML = Array.from(groups.entries()).map(([key, items]) => `
                    <tr class="lf-record-group"><td colspan="8">▸ ${key} <span style="opacity:.65;">${items.length}</span></td></tr>
            ...
            function construct() {
                ...
                const tableWrap = Ui.div({ style: 'margin:0 10px 10px;overflow:auto;flex:1;border:1px solid var(--item-border,#d7dce5);border-radius:6px;' });
                tableWrap.innerHTML = `
                    <style>
                        .lf-record-table { width:100%; border-collapse:collapse; font-size:13px; }
                        .lf-record-table th, .lf-record-table td { padding:7px 10px; border-bottom:1px solid rgba(148,163,184,.25); text-align:left; white-space:nowrap; }
                        .lf-record-table th { position:sticky; top:0; background:var(--card-title-background,#f8fafc); z-index:1; }
                        .lf-record-group td { background:rgba(148,163,184,.12); font-weight:700; }
                    </style>
    ...
    const BoatHelper = new class {
        ...
        appendFishInfo(content, fishInfo) {
            ...
            if (fishInfo.waterLayer) {
                ...
                if (s) this.appendFishInfoBadge(content, s.label, s.bg);
            }

            if (fishInfo.baitPreference) {
                const s = this.STYLE.BAIT[fishInfo.baitPreference];
                if (s) this.appendFishInfoBadge(content, s.label, this.TAG_BAIT_BG);
            } else if (fishInfo._dbBaitLabel) {
                this.appendFishInfoBadge(content, fishInfo._dbBaitLabel, this.TAG_BAIT_BG);
            }

            if (fishInfo.lurePreference) {
                const s = this.STYLE.LURE[fishInfo.lurePreference];
                if (s) this.appendFishInfoBadge(content, s.label, this.TAG_LURE_BG);
            } else if (fishInfo._dbLureLabel) {
                this.appendFishInfoBadge(content, fishInfo._dbLureLabel, this.TAG_LURE_BG);
            }
        ...
        resolveFishCardContent(node) {
            ...
            if (!fishInfo) { this._resolvedCache.set(node, null); return null; }
    ...
    const CatchTiming = new class {
        ...
        /**
         * 评分特征标记：
         * - 狠活（蓝）：经验宝宝(≈1%) / 99.01 刚越线进罕见 / 99.96 刚越线进传说
         * - 败北（红）：5~6% 不达标 / 98~99% 稀有(差一点上罕见) / 99.85~99.94 罕见(差一点上传说)
         */
        specialCatchMark(score) {
            if (typeof score !== 'number' || !Number.isFinite(score)) return null;
            // 狠活（蓝）
            if (score <= this.EXP_BABY_SCORE_MAX) return { text: '狠活', color: '#3b82f6' };
            if (score >= 99.00 && score <= 99.02) return { text: '狠活', color: '#3b82f6' };
            if (score >= 99.95 && score <= 99.97) return { text: '狠活', color: '#3b82f6' };
            // 败北（红）
            if (score >= 5 && score <= 6) return { text: '败北', color: '#ef4444' };
            if (score >= 98 && score < 99) return { text: '败北', color: '#ef4444' };
            if (score >= 99.85 && score <= 99.94) return { text: '败北', color: '#ef4444' };
            return null;
        }
        ...
        applySpecialCatchMark(card, parsed) {
            ...
            let badge = titleEl.querySelector(':scope > .lf-special-mark');
            if (!mark) {
                badge?.remove();
                return;
            }
            if (!badge) {
                ...
                badge.style.cssText = 'margin-left:6px;padding:1px 7px;border-radius:8px;font-size:12px;font-weight:700;color:#fff;vertical-align:middle;';
...
function initCatchSummary() {
    (function() {
        ...
        const EXP_BABY_SCORE_MAX = 2;
        const HENHUO_COLOR = '#3b82f6';
        const BAIBEI_COLOR = '#ef4444';

        function specialCatchMark(score) {
            if (typeof score !== 'number' || !Number.isFinite(score)) return null;
            if (score <= EXP_BABY_SCORE_MAX) return '狠活';
            if (score >= 99.00 && score <= 99.02) return '狠活';
            if (score >= 99.95 && score <= 99.97) return '狠活';
            if (score >= 5 && score <= 6) return '败北';
            if (score >= 98 && score < 99) return '败北';
            if (score >= 99.85 && score <= 99.94) return '败北';
            return null;
        }
            ...
            if (stats.allFish.length > 0) {
                ...
                stats.allFish.forEach((f, i) => {
                    ...
                    if (pct > 0) {
                        ...
                        seg.title = `${f.label}${f.mark ?? ''}: ${f.count}条`;
                ...
                fishLabels.innerHTML = stats.allFish.map((f, i) => {
                    const markColor = f.mark === '狠活' ? HENHUO_COLOR : (f.mark === '败北' ? BAIBEI_COLOR : FISH_COLORS[i % FISH_COLORS.length]);
                    const displayName = f.mark ? `${f.label}${f.mark}` : f.label;
                    return `<span><span style="color:${markColor};font-weight:600;">${displayName}</span> <span class="text-muted">${f.count}</span></span>`;
                }).join('');
