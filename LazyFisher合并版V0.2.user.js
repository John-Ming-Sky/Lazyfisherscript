    ...
    const FishKeepHistoryUi = new class {
        ...
        statPanel() {
            ...
            function refresh() {
                ...
                content.appendChild(statsTable('按鱼种统计', aggregate(rows, row => row.name)));
                content.appendChild(Ui.div({ style: 'height:10px;' }));
                content.appendChild(statsTable('按地点统计', aggregate(rows, row => row.region || '未知地点')));
            }

            function construct() {
                content = Ui.div({ style: 'margin:10px;display:flex;flex-direction:column;gap:0;' });
                refresh();
                return content;
            }
...
const FISH_DATABASE = [
    ...
    function isEnabled(featureId) {
        if (featureId === 'catchInterval' && window.LazyFisherMerged?.isEnabled('infoEnhance')) {
            return false;
        }
    ...
    function isHandledByInfoEnhance(featureId) {
        return featureId === 'catchInterval' && window.LazyFisherMerged?.isEnabled('infoEnhance');
    }
    ...
    function toggleFeature(featureId) {
        if (isHandledByInfoEnhance(featureId)) {
            alert('鱼口时间计算已由 LazyFisher信息增强 接管。公开版同名模块已停用，以避免鱼口/估价反复闪烁。');
            return;
        }
