    ...
    (function() {
        ...
        class FishCardEnhancer {
            ...
            static applyToCard(card) {
                // 可遇鱼类网格由信息增强 BoatHelper 统一渲染标签，避免重复/样式冲突
                if (card.closest('.region-fish-grid')) return;
                ...
                if (!info) return;
                if (card.querySelector('.fish-tag-row, .lf-fish-tag-row')) return;
                ...
                if (hasLayer || hasBait || hasLure) {
                    ...
                    if (hasBait) addBadge(getBaitShort(hasBait), 'bait', hasBait);
                    if (hasLure) addBadge(getLureShort(hasLure), 'lure', hasLure);
