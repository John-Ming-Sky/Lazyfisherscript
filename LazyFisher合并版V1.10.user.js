...
(async function () {
    ...
    const BoatHelper = new class {
        ...
        /** 页面上是否存在可见的可遇鱼类网格（含钓位选择弹窗） */
        hasVisibleRegionFishGrid() {
        ...
        clearFishSortPanelIfUnused() {
            if (this.hasVisibleRegionFishGrid()) return;
            const panel = document.getElementById('lf-boat-fish-sort-panel');
            if (panel) panel.remove();
            this._sortPanelEl = null;
            this._activeSortFishGrid = null;
        }
        ...
        ensureFishSortPanel(fishGrid) {
            ...
            let panel = this._sortPanelEl;
            if (!panel?.isConnected) {
                panel = document.getElementById('lf-boat-fish-sort-panel');
            }
            if (!panel?.isConnected) {
                panel = this.createFishSortPanel();
            } else if (panel.nextElementSibling !== grid || panel.parentNode !== parent) {
        ...
        tryRender() {
            ...
            if (!hasBoatMembers && !ownedGrid) {
                this.clearFishSortPanelIfUnused();
                return false;
            }
